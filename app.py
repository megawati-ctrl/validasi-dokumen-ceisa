...
def read_excel_smart(uploaded_file):
    """Membaca file Excel biner maupun file HTML/CSV berkedok .xls"""
    bytes_data = uploaded_file.read()
    uploaded_file.seek(0)
    
    # Coba baca sebagai Excel standar lebih dulu
    try:
        return pd.read_excel(io.BytesIO(bytes_data))
    except Exception:
        pass

    # Jika gagal (seperti error Workbook corruption), coba baca sebagai HTML Table
    try:
    ...
    # Jika masih gagal, coba baca sebagai CSV
    try:
        return pd.read_csv(io.BytesIO(bytes_data))
    except Exception:
        pass

    raise ValueError("File tidak dapat dibaca. Format file tidak sesuai ekstensi.")
...
if st.button("🚀 Jalankan Validasi", type="primary"):
    if file_inv and file_pl and file_ceisa:
        try:
            ...
            type_inv, data_inv, qty_inv, gw_inv = (res_inv[0], None, res_inv[1], res_inv[2]) if res_inv[0] == "pdf" else (res_inv[0], res_inv[1], res_inv[2], res_inv[3])
            type_pl, data_pl, qty_pl, gw_pl = (res_pl[0], None, res_pl[1], res_pl[2]) if res_pl[0] == "pdf" else (res_pl[0], res_pl[1], res_pl[2], res_pl[3])
            type_ceisa, data_ceisa, qty_ceisa, gw_ceisa = (res_ceisa[0], None, res_ceisa[1], res_ceisa[2]) if res_ceisa[0] == "pdf" else (res_ceisa[0], res_ceisa[1], res_ceisa[2], res_ceisa[3])
            ...
            with st.expander("Lihat Detail Data Uploaded"):
                ...
                with col_a:
                    ...
                    st.dataframe(data_inv) if type_inv == "table" else st.text("Konten PDF berhasil dibaca")
                with col_b:
                    st.subheader("Detail Packing List")
                    st.subheader("Detail Packing List")
                    st.dataframe(data_pl) if type_pl == "table" else st.text("Konten PDF berhasil dibaca")
                with col_c:
                    st.subheader("Detail CEISA")
                    st.dataframe(data_ceisa) if type_ceisa == "table" else st.text("Konten PDF berhasil dibaca")
